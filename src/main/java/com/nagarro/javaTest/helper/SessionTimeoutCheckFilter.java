...
public class SessionTimeoutCheckFilter implements Filter {
	...
	Logger log = LoggerFactory.getLogger(SessionTimeoutCheckFilter.class);
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		if (session != null && session.getAttribute("lastAccessTime") != null) {
			...
			if (timeLeft <= 0) {
				log.trace("Session Expired. Session Id: {}", session.getId());
				resp.getWriter().print(timeLeft);
			} else {
				log.trace("Session valid. Time left: {}", timeLeft);
				resp.getWriter().print(timeLeft);
			}
		} else {
			log.warn("Request received without a valid session");
